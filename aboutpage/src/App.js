...
class AboutPage extends React.Component {
  constructor(props) {
    super(props);
    this.index = 0; //Variable for indexing arrays
    this.imageArray = [
      //Arrays for charity images
      "https://i1.wp.com/metro.co.uk/wp-content/uploads/2018/11/sei_40316346-18d6.jpg?quality=90&strip=all&zoom=1&resize=644%2C338&ssl=1",
    ...
    this.descriptionArray = [
      //Array for message images
      "Charity 1, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus nec ligula vel augue blandit mollis nec a sem. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Donec vestibulum congue eros euismod scelerisque. Vestibulum.",
  ...
  rightClick = () => {
    ...
    console.log("right click 🔥, index is " + this.sindex);
    this.setState({ text: this.descriptionArray[this.index] });
    document.getElementById("picture").src = this.imageArray[this.index];
  };

  render() {
    //Component renders from here
    return (
      <div id="root" className="About">
        <header className="App-header">
          <img
            ...
            className="logo"
            alt="Ava's Angels logo"
          />

          <img
            id="picture"
            src={this.imageArray[this.index]}
            className="picture"
            alt="Bunk bar"
          />

          <button onClick={this.rightClick} className="right-button">
            {" "}
            >{" "}
          </button>
          <button onClick={this.leftClick} className="left-button">
            {" "}
            &lt;{" "}
          </button>
